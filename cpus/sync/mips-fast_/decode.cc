...
//Trying to use flags instead of else if's.... //need to add float reg bypasses??
void
Decode::MainLoop (void)
{
   ...
   while (1) {
      if (1) {
         ...
         bool prev_ex_decoded_dst = _mc->_de->_decodedDST;
         bool prev_me_write_reg = _mc->_em->_writeREG;
         bool prev_me_decoded_dst = _mc->_em->_decodedDST;
         ...
         if (_mc->_de->rs_num && prev_ex_mem_control == FALSE) {
            if (prev_ex_write_reg && prev_ex_decoded_dst == _mc->_de->rs_num) _mc->_de->EX_EX_1= TRUE;
         }
         ...
         //MEMEX
         if (_mc->_de->rs_num) {
            if (prev_me_write_reg && prev_me_decoded_dst == _mc->_de->rs_num) _mc->_de->MEM_EX_1= TRUE;
         }
         if (_mc->_de->rt_num) {
