...
EM::func_lb (EM *mc, unsigned ins)
{
   ...
   mc->_num_load++;
   SIGN_EXTEND_IMM(mc->_decodedSRC2);
   mc->_MAR = (unsigned)(mc->_decodedSRC1+mc->_decodedSRC2);
}
...
{
   mc->_mem->Write(mw->_MAR & ~(LL)0x7, mc->_mem->BESetWord (mw->_MAR, mc->_mem->Read(mw->_MAR & ~(LL)0x7), mc->_fpr[mw->_decodedDST>>1].l[FP_TWIDDLE^(mw->_decodedDST&1)]));
}
...
{
   mc->_mem->Write(mw->_MAR & ~(LL)0x7, mc->_mem->BESetByte (mw->_MAR, mc->_mem->Read(mw->_MAR & ~(LL)0x7), mc->_gpr[mw->_decodedDST] & 0xff));
}

void
MW::mem_sh (Mipc *mc, MW *mw)
{
   mc->_mem->Write(mw->_MAR & ~(LL)0x7, mc->_mem->BESetHalfWord (mw->_MAR, mc->_mem->Read(mw->_MAR & ~(LL)0x7), mc->_gpr[mw->_decodedDST] & 0xffff));
}
...
{
   mc->_mem->Write(mw->_MAR & ~(LL)0x7, mc->_mem->BESetWord (mw->_MAR, mc->_mem->Read(mw->_MAR & ~(LL)0x7), mc->_gpr[mw->_decodedDST]));
}
