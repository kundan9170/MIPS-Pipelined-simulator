   ...
   while (1) {
      if (1) {
        ...
        EM *em= new EM(_mc->_de);
         //bool change_pc = _mc->_de->change_pc;
         if(em->rs_num!=0 && em->MEM_MEM_1==TRUE) em->_decodedSRC1 =_mc->_mw->_opResultLo;
         if(em->rt_num!=0 && em->MEM_MEM_2==TRUE) em->_decodedSRC2 =_mc->_mw->_opResultLo;
         unsigned int ins;
         ins=em->_ins;
         isSyscall=em->_isSyscall;
         isIllegalOp=em->_isIllegalOp;
         if(em->rs_num!=0){
            if(_mc->dest_state[em->rs_num] == _EX_){
               int take = _mc->from_EM[em->rs_num];
               if(take == TAKE_EM)em->_decodedSRC1 = _mc->_em->_opResultLo;
               else if(take == TAKE_MW)em->_decodedSRC1 = _mc->_mw->_opResultLo;
            }
            else{
               int take = _mc->from_MW[em->rs_num];
               if(take == TAKE_MW)em->_decodedSRC1 = _mc->_mw->_opResultLo;
            }
         }
         if(em->rt_num!=0){
            if(_mc->dest_state[em->rt_num] == _EX_){
               int take = _mc->from_EM[em->rt_num];
               if(take == TAKE_EM)em->_decodedSRC2 = _mc->_em->_opResultLo;
               else if(take == TAKE_MW)em->_decodedSRC2 = _mc->_mw->_opResultLo;
            }
            else{
               int take = _mc->from_MW[em->rs_num];
               if(take == TAKE_MW)em->_decodedSRC2 = _mc->_mw->_opResultLo;
            }
         }
